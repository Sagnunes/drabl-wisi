...
const confirmDelete = () => {
    form.delete(`/statuses/${statusToDelete.value.id}`, {
        onSuccess: () => {
            showDeleteDialog.value = false;
                    ...
                    <div class="-mx-4 -my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                        <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                            <div class="overflow-hidden shadow-sm ring-1 ring-black/5 sm:rounded-lg">
                                <SimpleDataTable :columns :data="statuses" @delete="handleDeleteDialog" />
                                        ...
                                        <DialogHeader>
                                            ...
                                            <DialogDescription>
                                                <div class="flex flex-col">
                                                    <p class="text-base">Tem a certeza que deseja apagar o perfil "{{ statusToDelete?.name }}"?</p>
                                                    <span class="text-base underline">Está acção não pode ser desfeita.</span>
                                                </div>
                                                <p v-show="form.errors.status" class="mt-2 text-sm text-red-600 dark:text-red-500">{{ form.errors.status }}</p>
                                            </DialogDescription>
                                        </DialogHeader>
                                        <DialogFooter>
